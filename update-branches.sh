...
# Per-version branch orchestrator for kapowarr-flake. See bencoding-flake/update-branches.sh for the general design.
#
# Kapowarr-specific bits:
# - list_upstream_versions queries GitHub releases (Casvt/Kapowarr).
# - placeholder pin.nix has 3 fields (version, sourceRev, sourceHash).
# - flake.nix is branch-owned (update-version rewrites the bencoding URL line per upstream constraint) and included in the diff check.
#
# Structural: each existing exact branch is `git merge`d with origin/main before its update-version runs, so orchestrator/workflow improvements that land on main propagate forward through every branch's tree. Branch-owned files stay as-is via .gitattributes' `merge=ours` rules.
#
...
list_upstream_versions() {
  gh api --paginate "/repos/Casvt/Kapowarr/releases" --jq '.[].tag_name'
}
...
for v in "${tracked[@]}"; do
  ...
  if ! git diff --quiet -- pin.nix flake.nix flake.lock || [[ -n "$(git ls-files --others --exclude-standard -- flake.lock)" ]]; then
    git add pin.nix flake.nix flake.lock
